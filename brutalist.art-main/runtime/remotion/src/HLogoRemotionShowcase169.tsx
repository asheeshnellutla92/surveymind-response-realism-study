/**
 * HLogoRemotionShowcase169 — every Remotion move, one logo. (16:9 landscape)
 *
 ...
 *
 * Derived from HLogoRemotionShowcase.tsx (9:16 portrait).
 * Changes from 9:16 source:
 *   - Composition dimensions: 1920×1080 (16:9)
 *   - KineticGrid: uses useVideoConfig() width/height instead of hardcoded 1080/1920
 *   - All other technique logic, spring configs, and audio are UNCHANGED
 *
 * Audio: reuses public/h-logo-mp3/* from the 9:16 build — not regenerated.
 *
 ...
 * B11  Kinetic Grid (layout adapted for 16:9)
 * B12  Glitch Slices
 * B13  Trail Echo
 * B14  Noise Wobble
 * B15  Elastic Physics
...
const TechniqueBeat: React.FC<TechniqueBeatProps> = ({ label, sparkLine, children }) => {
  ...
  return (
    <AbsoluteFill style={{ background: CLAUDE.PAGE, overflow: 'hidden' }}>
      ...
      {/* Mark stage — centered in the middle 70% of the 16:9 canvas */}
      <div style={{
        position: 'absolute',
        top: '18%', bottom: '18%',
        left: 0, right: 0,
...
const MaskReveal: React.FC = () => {
  ...
  const clipId1 = 'mask-linear-169';
  const clipId2 = 'mask-radial-169';
...
// ── B11 Kinetic Grid (16:9 layout) ────────────────────────────────────────────
const KineticGrid: React.FC = () => {
  const frame = useCurrentFrame();
  const fps = 30;
  // Use actual canvas dimensions from composition config (works for both 9:16 and 16:9)
            ...
            {Array.from({ length: slices }, (_, i) => {
              ...
              return (
                <g key={i} style={{ transform: `translateX(${offsetX}px)` }}>
                  <clipPath id={`slice-169-${i}`}>
                    <rect x={0} y={clipY} width={LOGO_W} height={clipH} />
                  </clipPath>
                  <g clipPath={`url(#slice-169-${i})`}>
  ...
  return (
    <TechniqueBeat label="Orbit Parts" sparkLine="Reunion lands with weight.">
      <div style={{ position: 'relative', width: w + 200, height: h + 100 }}>
        ...
        >
          <path d={P1} fill="#171717"
            transform={`translate(${sepX1 + orbitX1}, ${orbitY1})`}
          />
          <path d={P2} fill={CLAUDE.SPARK}
            transform={`translate(${sepX2 + orbitX2}, ${orbitY2})`}
          />
        </svg>
      </div>
    </TechniqueBeat>
...
const ComposerSummon: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ background: CLAUDE.PAGE, overflow: 'hidden' }}>
      ...
      {/* Composer card — centered horizontally, upper half of canvas */}
      <div style={{
      ...
      {/* Mark arriving — lower half of canvas */}
      <div style={{
...
// ── Timing (shared with 9:16 — same audio, same frame counts) ─────────────────
const TIMED = TIMING.map((t) => ({ ...t }));
export const TOTAL_FRAMES = TIMED.reduce((a, b) => a + b.frames, 0);

// ── Main composition ──────────────────────────────────────────────────────────
export const HLogoRemotionShowcase169: React.FC = () => {
  let at = 0;
