...
/**
 * ConfigureTell — B05 — Teardown: gets right / bites.
 */

export const configureTellSchema = z.object({
  sparkLine: z.string().default('Lockdown rule and credential hygiene solid. Token validation and schema gaps: surface them.'),
});
export type ConfigureTellProps = z.infer<typeof configureTellSchema>;
...
const GETS_RIGHT = [
  ...
  'Push-toward-lockdown is explicit and proactive: offer the allowlist flip without waiting to be asked',
  'chmod 600 on .env is built into the token-save flow — credential hygiene is mandatory, not optional',
  '.env restart vs access.json immediate: the two restart behaviors are clearly contrasted in the skill',
  'Next-step guidance for each of the three states is a specific command, not just a description of state',
];

const BITES = [
  'No token format validation before saving — any argument string is written to .env without checking token shape',
  '"Preserve other keys" is stated but the skill never says what other valid .env keys exist',
  'After saving a new token, user is told to restart, but no restart command or verification step is given',
  'access.json described by example (dmPolicy, allowlist, pending pairings) — full schema never stated',
  'Discord\'s own gates (shared-server requirement, Public Bot toggle) mentioned as context but never explained',
];

export const ConfigureTell: React.FC<ConfigureTellProps> = ({ sparkLine }) => {
  ...
  return (
    <AbsoluteFill style={{ background: CLAUDE.PAGE }}>
      ...
      <div style={{
        ...
        Lockdown rule solid. Token validation gap: surface it.
      </div>
      ...
      <div style={{
        ...
        <div style={{ fontFamily: SANS, fontSize: 16, color: CLAUDE.INK, lineHeight: 1.6 }}>
          A user who saves a new token and doesn't restart will think the configuration worked — but the server is still running the{' '}
          <span style={{ fontFamily: MONO, color: CLAUDE.SPARK, fontWeight: 700 }}>old token</span>
          . The skill says to restart but gives no command to do it.
        </div>
      </div>
