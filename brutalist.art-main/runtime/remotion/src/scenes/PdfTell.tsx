...
/**
 * PdfTell — B05 — Teardown moment.
 * Central callout: library router insight. Two columns: gets right / bites.
 */

export const pdfTellSchema = z.object({
  sparkLine: z.string().default('Three libraries. One routing rule. Two specialist files.'),
});
export type PdfTellProps = z.infer<typeof pdfTellSchema>;
...
const GETS_RIGHT = [
  'Clean library routing: manipulation/extraction/creation each own their domain',
  'OCR pipeline is complete: pdf2image converts first, pytesseract recognizes second',
  'reportlab Unicode subscript gotcha is explicitly called out — saves real debugging',
  'CLI tools available for each operation as an alternative to Python',
  'Specialist files (FORMS.md, REFERENCE.md) keep scope clean for the main skill',
];

const BITES = [
  'Form filling and advanced features delegated to separate files — must load them first',
  'No guidance on PDF accessibility, tagging, or digital signatures',
  'pdfplumber table extraction requires manual column-alignment tuning on complex tables',
  'reportlab creation can be verbose — no mention of simpler alternatives like weasyprint',
  'OCR quality depends on tesseract install and PDF scan quality — skill does not address',
];

export const PdfTell: React.FC<PdfTellProps> = ({ sparkLine }) => {
  ...
  return (
    <AbsoluteFill style={{ background: CLAUDE.PAGE }}>
      ...
      <div style={{
        ...
        Manipulate. Extract. Create. Know which.
      </div>
      ...
      <div style={{
        ...
        <div style={{ fontFamily: SANS, fontSize: 17, color: CLAUDE.INK, lineHeight: 1.6 }}>
          The architectural insight is that the PDF skill is a{' '}
          <span style={{ fontFamily: MONO, color: CLAUDE.SPARK, fontWeight: 700 }}>library router</span>:{' '}
          three Python libraries each own a domain — manipulation, extraction, creation —
          plus CLI tools, and two specialist files that handle edge cases the main skill delegates away.
        </div>
      </div>
